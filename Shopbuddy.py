...
st.set_page_config(page_title="Product IQ: Agentic Shopper", page_icon="🧠", layout="wide")
...
if not api_key:
    ...
    with st.sidebar:
        ...
        if api_key_input:
            ...
            st.warning("⚠️ API Key needed to run.")
            st.stop()
...
if api_key:
    ...
    # This case should ideally be caught by st.stop() above, but for safety
...
st.title("🛍️ Product IQ: The Agentic Shopper")
st.markdown("### Zero Bias. Deep Intelligence. Total Transparency.")
...
with st.form("research_form"):
    ...
    submitted = st.form_submit_button("🚀 Run Product Intelligence Engine")
...
if submitted and product_input:
    ...
    status = st.status("🕵️ Initiating Deep Dive Agents...", expanded=True)

    try:
        # 1. Research
        status.write("🌍 **Agent 1 (The Hunter):** Scouring global markets, rivals, and forums...")
        research_response = run_research(product_input)
        st.session_state.research_data = research_response.text

        # 2. Report
        status.write("🧠 **Agent 2 (The Analyst):** Detecting bias, calculating value scores...")
...
if st.session_state.general_report:
    ...
    st.markdown("## 👤 Phase 2: User Compatibility Engine")
    st.markdown("The report above is for *everyone*. Now, let's see if this product is for **YOU**.")
    ...
    st.markdown("## 💬 Ask Me Anything (Real-Time)")
    st.caption(f"Chatting with the Product Intelligence Engine about **{st.session_state.product_name}**")
